...
namespace ArduinoSerialServer
{
    public static class SocketServer
    {
        ...
        private static void LaunchServer()
        {
            ...
            _parent.AddMessage("My hostname is: " + hostname);
            //IPHostEntry ipHostInfo = Dns.Resolve(Dns.GetHostName());
            IPHostEntry ipHostInfo = Dns.GetHostEntry(hostname);
            IPAddress ipAddress = ipHostInfo.AddressList.Where(i => i.AddressFamily == AddressFamily.InterNetwork).First();
#else
            IPAddress ipAddress = new IPAddress(new byte[] { 127, 0, 0, 1 });
#endif
            _parent.AddMessage("Endpoint: " + ipAddress.ToString() + ":" + _portNumber.ToString());
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    _parent.AddMessage("Waiting for a connection...");
        ...
        private static void AcceptCallback(IAsyncResult ar)
        {
            ...
            if ((_mainConnectionHandler != null) && (_mainConnectionHandler != handler))
            {
                _mainConnectionHandler.Close();
            }
            _mainConnectionHandler = handler;
